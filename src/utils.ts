...
import { StockConfig } from "./features";
...
export function twseApi(stockConfig: StockConfig): Promise<Array<Stock>> {
  const twseUrlPrefix =
  ...
  return new Promise(async (resolve) => {
    const twseRetData = await httpRequest(searchTickerUrl);
